...
#include "LED_Drive.h"
#include <LED_Drive_DMA_Com_dma.h>
#include <LED_Drive_DMA_Seg_dma.h>
#if (LED_Drive_BRIGHTNESS_EN == LED_Drive_ENABLED)
    #include <LED_Drive_DMA_BC_dma.h>
#endif /* (LED_Drive_BRIGHTNESS_EN == LED_Drive_ENABLED) */

/* Function prototypes */
static void LED_Drive_DmaInit(void) ;
static void LED_Drive_DmaEnable(void) ;
static void LED_Drive_DmaDisable(void) ;

/* Used to hold the common line output sequence */
#if(LED_Drive_COM_DRIVE == LED_Drive_HIGH)
    static uint8 LED_Drive_comBuffer[16] = {0x00u, 0x01u, 0x00u, 0x02u, 0x00u, 0x04u, 0x00u, 0x08u,
                                                    0x00u, 0x10u, 0x00u, 0x20u, 0x00u, 0x40u, 0x00u, 0x80u};
#else
    static uint8 LED_Drive_comBuffer[16] = {0xFFu, 0xFEu, 0xFFu, 0xFDu, 0xFFu, 0xFBu, 0xFFu, 0xF7u,
                                                    0xFFu, 0xEFu, 0xFFu, 0xDFu, 0xFFu, 0xBFu, 0xFFu, 0x7Fu};
#endif /* (LED_Drive_COM_DRIVE == LED_Drive_HIGH) */

/* Buffers used to hold the display(segment) values to be transferred to the control registers */
uint8 LED_Drive_segLBuffer[LED_Drive_NUM_COM];
#if (LED_Drive_NUM_SEG > 8u) 
    uint8 LED_Drive_segMBuffer[LED_Drive_NUM_COM];
#endif /* (LED_Drive_NUM_SEG > 8u) */
#if (LED_Drive_NUM_SEG > 16u) 
    uint8 LED_Drive_segHBuffer[LED_Drive_NUM_COM];
#endif /* (LED_Drive_NUM_SEG > 16u) */

#if (LED_Drive_BRIGHTNESS_EN == LED_Drive_ENABLED)
    /* The brightness register used to hold the brightness levels for each common */
    static uint8 LED_Drive_brightnessReg[LED_Drive_NUM_COM];
#endif /* (LED_Drive_BRIGHTNESS_EN == LED_Drive_ENABLED) */

/* DMA channel and TD declaractions */
static uint8 LED_Drive_dmaComChannel;
static uint8 LED_Drive_dmaComTd[LED_Drive_TWO_NUM_COM];

static uint8 LED_Drive_dmaSegChannel;
static uint8 LED_Drive_dmaSegLTd[LED_Drive_NUM_COM];

#if (LED_Drive_BRIGHTNESS_EN == LED_Drive_ENABLED)
    static uint8 LED_Drive_dmaBCChannel;
    static uint8 LED_Drive_dmaBCTd;
#endif /* (LED_Drive_BRIGHTNESS_EN == LED_Drive_ENABLED) */

uint8 LED_Drive_initVar = 0u;
uint8 LED_Drive_enableVar = 0u;


/*******************************************************************************
* Function Name: LED_Drive_DmaInit
********************************************************************************
*
* Summary: 
...
static void LED_Drive_DmaInit(void) 
{
    ...
    #if (LED_Drive_NUM_SEG > 8u)
        static uint8 LED_Drive_dmaSegMTd[LED_Drive_NUM_COM];
    #endif /* (LED_Drive_NUM_SEG > 8u) */
    #if (LED_Drive_NUM_SEG > 16u)
        static uint8 LED_Drive_dmaSegHTd[LED_Drive_NUM_COM];
    #endif /* (LED_Drive_NUM_SEG > 16u) */

    /************************* DMA configuration for the Commons *************************/
    /* Common DMA channel */
    LED_Drive_dmaComChannel = LED_Drive_DMA_Com_DmaInitialize(1u, 1u, 
                                        HI16(LED_Drive_SRAM_BASE), HI16(LED_Drive_CONTROL_REG_BASE));

    /* Check if the channel is valid */                                                                        
    if(LED_Drive_dmaComChannel != DMA_INVALID_CHANNEL)
    {
        /* Allocate the common TDs */
        for(i = 0u; i < LED_Drive_TWO_NUM_COM; i++)
        {
            LED_Drive_dmaComTd[i] = CyDmaTdAllocate();
        }
        
        /* Set configuration for the Common TDs */
        for(i = 0u;  i < (LED_Drive_TWO_NUM_COM - 1u); i++)
        {
            /* Separate configurations for odd and even TDs */
            if((uint8)(i & 1u) == 0u)
            {
                /* Set configuration for the Common TDs */
                (void) CyDmaTdSetConfiguration(LED_Drive_dmaComTd[i], 1u, 
                                                LED_Drive_dmaComTd[i+1u],
                                                TD_INC_SRC_ADR | LED_Drive_DMA_Com__TD_TERMOUT_EN);
            }
            else
            {
                /* Set configuration for the Common TDs */
                (void) CyDmaTdSetConfiguration(LED_Drive_dmaComTd[i], 1u,
                                                LED_Drive_dmaComTd[i+1u], TD_INC_SRC_ADR);
            }
        }
        /* Set the wraparound Common TD configuration */
        (void) CyDmaTdSetConfiguration(LED_Drive_dmaComTd[LED_Drive_TWO_NUM_COM - 1u], 
                                        1u, LED_Drive_dmaComTd[0u], TD_INC_SRC_ADR);
        
        /* Set the lower addresses of the common TDs */
        for(i = 0u; i < LED_Drive_TWO_NUM_COM; i++)
        {
            (void) CyDmaTdSetAddress(LED_Drive_dmaComTd[i],
                                        LO16((uint32) &LED_Drive_comBuffer[i]),
                                        LO16((uint32) LED_Drive_COM_CONTROL_PTR));
        }    
    }
    ...
    LED_Drive_dmaSegChannel = LED_Drive_DMA_Seg_DmaInitialize(1u, 1u, 
                                        HI16(LED_Drive_SRAM_BASE), HI16(LED_Drive_CONTROL_REG_BASE));
    
    /* Check if the segment channel is valid */
    if(LED_Drive_dmaSegChannel != DMA_INVALID_CHANNEL)
    {
        /* Allocate separate Tds for segments to implement the chain */
        for(i = 0u; i < LED_Drive_NUM_COM; i++)
        {
            LED_Drive_dmaSegLTd[i] = CyDmaTdAllocate();
            #if (LED_Drive_NUM_SEG > 8u)
                LED_Drive_dmaSegMTd[i] = CyDmaTdAllocate();
            #endif /* (LED_Drive_NUM_SEG > 8u) */
            #if (LED_Drive_NUM_SEG > 16u)
                LED_Drive_dmaSegHTd[i] = CyDmaTdAllocate();
            #endif /* (LED_Drive_NUM_SEG > 16u) */
        }
        
        /* Set the configuration for the Segment TDs */
        for(i = 0u; i < (LED_Drive_NUM_COM - 1u); i++)
        {
            #if (LED_Drive_NUM_SEG <= 8u)
                (void) CyDmaTdSetConfiguration(LED_Drive_dmaSegLTd[i], 1u, LED_Drive_dmaSegLTd[i+1u],
                                                TD_INC_SRC_ADR | LED_Drive_DMA_Seg__TD_TERMOUT_EN);
            #else    
                (void) CyDmaTdSetConfiguration(LED_Drive_dmaSegLTd[i], 1u, LED_Drive_dmaSegMTd[i],
                                                TD_INC_SRC_ADR | TD_AUTO_EXEC_NEXT);

                #if (LED_Drive_NUM_SEG <= 16u)
                    (void) CyDmaTdSetConfiguration(LED_Drive_dmaSegMTd[i], 1u, LED_Drive_dmaSegLTd[i+1u],
                                                    TD_INC_SRC_ADR | LED_Drive_DMA_Seg__TD_TERMOUT_EN);
                #else    
                    (void) CyDmaTdSetConfiguration(LED_Drive_dmaSegMTd[i], 1u, LED_Drive_dmaSegHTd[i],
                                                    TD_INC_SRC_ADR | TD_AUTO_EXEC_NEXT);
                    (void) CyDmaTdSetConfiguration(LED_Drive_dmaSegHTd[i], 1u, LED_Drive_dmaSegLTd[i+1u],
                                                    TD_INC_SRC_ADR | LED_Drive_DMA_Seg__TD_TERMOUT_EN);
                #endif /* LED_Drive_NUM_SEG <= 16u */
            #endif /* LED_Drive_NUM_SEG <= 8u */
        }
        ...
        #if (LED_Drive_NUM_SEG <= 8u)
            (void) CyDmaTdSetConfiguration(LED_Drive_dmaSegLTd[LED_Drive_NUM_COM - 1u],
                                            1u, LED_Drive_dmaSegLTd[0u], 
                                            TD_INC_SRC_ADR | LED_Drive_DMA_Seg__TD_TERMOUT_EN);
        #else    
            (void) CyDmaTdSetConfiguration(LED_Drive_dmaSegLTd[LED_Drive_NUM_COM - 1u],
                                            1u, LED_Drive_dmaSegMTd[LED_Drive_NUM_COM - 1u],
                                            TD_INC_SRC_ADR | TD_AUTO_EXEC_NEXT);
            #if (LED_Drive_NUM_SEG <= 16u)
                (void) CyDmaTdSetConfiguration(LED_Drive_dmaSegMTd[LED_Drive_NUM_COM - 1u],
                                                1u, LED_Drive_dmaSegLTd[0u], 
                                                TD_INC_SRC_ADR | LED_Drive_DMA_Seg__TD_TERMOUT_EN);
            #else    
                (void) CyDmaTdSetConfiguration(LED_Drive_dmaSegMTd[LED_Drive_NUM_COM - 1u],
                                                1u, LED_Drive_dmaSegHTd[LED_Drive_NUM_COM - 1u], 
                                                TD_INC_SRC_ADR | TD_AUTO_EXEC_NEXT);
                (void) CyDmaTdSetConfiguration(LED_Drive_dmaSegHTd[LED_Drive_NUM_COM - 1u],
                                                1u, LED_Drive_dmaSegLTd[0u], 
                                                TD_INC_SRC_ADR | LED_Drive_DMA_Seg__TD_TERMOUT_EN);
            #endif /* (LED_Drive_NUM_SEG <= 16u) */
        #endif /* (LED_Drive_NUM_SEG <= 8u) */
        ...
        for(i = 0u; i < LED_Drive_NUM_COM; i++)
        {
            (void) CyDmaTdSetAddress(LED_Drive_dmaSegLTd[i],
                                            LO16((uint32) &LED_Drive_segLBuffer[i]),
                                            LO16((uint32) LED_Drive_SEG_L_CONTROL_PTR));
            #if (LED_Drive_NUM_SEG > 8u)
                (void) CyDmaTdSetAddress(LED_Drive_dmaSegMTd[i],
                                            LO16((uint32) &LED_Drive_segMBuffer[i]),
                                            LO16((uint32) LED_Drive_SEG_M_CONTROL_PTR));
            #endif /* (LED_Drive_NUM_SEG > 8u) */
            #if (LED_Drive_NUM_SEG > 16u)
                (void) CyDmaTdSetAddress(LED_Drive_dmaSegHTd[i], 
                                            LO16((uint32) &LED_Drive_segHBuffer[i]),
                                            LO16((uint32) LED_Drive_SEG_H_CONTROL_PTR));
            #endif /* (LED_Drive_NUM_SEG > 16u) */
        }
    }
    ...
    /******************** DMA configuration for Brightness control *********************/
        ...
        LED_Drive_dmaBCChannel = LED_Drive_DMA_BC_DmaInitialize(1u, 1u, 
                                        HI16(LED_Drive_SRAM_BASE), HI16(LED_Drive_CONTROL_REG_BASE));
            
        /* Check if the channels are valid */
        if(LED_Drive_dmaBCChannel != DMA_INVALID_CHANNEL)
        {
            LED_Drive_dmaBCTd = CyDmaTdAllocate();
            
            /* Set the TD configuration for the brightness DMA */
            (void) CyDmaTdSetConfiguration(LED_Drive_dmaBCTd, LED_Drive_NUM_COM,
                                        LED_Drive_dmaBCTd, TD_INC_SRC_ADR);
            
            /* Lower address for the brightness Td */
            (void) CyDmaTdSetAddress(LED_Drive_dmaBCTd, LO16((uint32) &LED_Drive_brightnessReg[0u]),
                                                               LO16((uint32)LED_Drive_COMPARE_PTR));
        }
    #endif /* (LED_Drive_BRIGHTNESS_EN == LED_Drive_ENABLED) */
}
...
/*******************************************************************************
* Function Name: LED_Drive_DmaEnable
********************************************************************************
*
* Summary: 
...
static void LED_Drive_DmaEnable(void) 
{
    ...
    (void) CyDmaChSetInitialTd(LED_Drive_dmaComChannel, LED_Drive_dmaComTd[0u]);
    (void) CyDmaChSetInitialTd(LED_Drive_dmaSegChannel, LED_Drive_dmaSegLTd[0u]);
    #if (LED_Drive_BRIGHTNESS_EN == LED_Drive_ENABLED)
        (void) CyDmaChSetInitialTd(LED_Drive_dmaBCChannel, LED_Drive_dmaBCTd);
    #endif /* (LED_Drive_BRIGHTNESS_EN == LED_Drive_ENABLED) */
    
    /* Enable all channels */
    (void) CyDmaChEnable(LED_Drive_dmaComChannel, 1u);
    (void) CyDmaChEnable(LED_Drive_dmaSegChannel, 1u);
    #if (LED_Drive_BRIGHTNESS_EN == LED_Drive_ENABLED)
        (void) CyDmaChEnable(LED_Drive_dmaBCChannel, 1u);
    #endif /* (LED_Drive_BRIGHTNESS_EN == LED_Drive_ENABLED) */
}


/*******************************************************************************
* Function Name: LED_Drive_DmaDisable
********************************************************************************
*
* Summary: 
...
static void LED_Drive_DmaDisable(void) 
{
    uint8 i;
    uint8 channel;
    uint8 dmaChannelList[LED_Drive_CHANNEL_LIMIT];
    
    dmaChannelList[0u] = LED_Drive_dmaComChannel;
    dmaChannelList[1u] = LED_Drive_dmaSegChannel;
    #if (LED_Drive_BRIGHTNESS_EN == LED_Drive_ENABLED)
        dmaChannelList[2u] = LED_Drive_dmaBCChannel;
    #endif /* (LED_Drive_BRIGHTNESS_EN == LED_Drive_ENABLED) */

    for(i = 0u; i < LED_Drive_CHANNEL_LIMIT; i++)
    {
        channel = dmaChannelList[i];
        (void) CyDmaChDisable(channel);
        
        LED_Drive_Com_Driver_Write(LED_Drive_CLEAR);
...
{
    static uint8 LED_Drive_dmaInitDone = 0u;
    #if (LED_Drive_BRIGHTNESS_EN == LED_Drive_ENABLED)
        uint8 i;
    #endif /* (LED_Drive_BRIGHTNESS_EN == LED_Drive_ENABLED) */
    
    /* Start fresh by clearing the display */
    LED_Drive_ClearDisplayAll();
    
    /* Initialize the DMAs */
    if(0u == LED_Drive_dmaInitDone)
    {
        LED_Drive_DmaInit();
        LED_Drive_dmaInitDone = 1u;
    }
    ...
    /* Set the brightness to full */
        ...
        for(i = 0u; i<(LED_Drive_NUM_COM); i++)
        {
            LED_Drive_brightnessReg[i] = LED_Drive_INIT_BRIGHT;
        }
    #endif /* (LED_Drive_BRIGHTNESS_EN == LED_Drive_ENABLED) */
}
...
/*******************************************************************************
* Function Name: LED_Drive_Enable
********************************************************************************
*
* Summary: 
...
void LED_Drive_Enable(void) 
{
    ...
    #if (LED_Drive_BRIGHTNESS_EN == LED_Drive_ENABLED)
       /* Initialize to zero */
       LED_Drive_COUNTER_REG = 0x00u;
       LED_Drive_COMPARE_REG = 0x00u;
       
        /* Enable the PWM for brightness control */
       LED_Drive_CONTROL_REG = LED_Drive_CTRL_ENABLE;
    #endif /* (LED_Drive_BRIGHTNESS_EN == LED_Drive_ENABLED) */
    
    LED_Drive_enableVar = LED_Drive_CTRL_ENABLE;
}


/*******************************************************************************
* Function Name: LED_Drive_Start
********************************************************************************
*
* Summary:
...
{
    if(0u == LED_Drive_initVar)
    {
        LED_Drive_Init();
        LED_Drive_initVar = 1u;
    }
...
void LED_Drive_Stop(void) 
{
    #if (LED_Drive_BRIGHTNESS_EN == LED_Drive_ENABLED)
        /* Disable the PWM */
        LED_Drive_CONTROL_REG = LED_Drive_CTRL_DISABLE;
    #endif /* (LED_Drive_BRIGHTNESS_EN == LED_Drive_ENABLED) */
    
    LED_Drive_DmaDisable();
    LED_Drive_ClearDisplayAll();
    
    LED_Drive_enableVar  = LED_Drive_CTRL_DISABLE;
}


/*******************************************************************************
* Function Name: LED_Drive_SetDisplayRAM
********************************************************************************
*
* Summary:
...
*  RAM directly by writing to LED_Drive_segLBuffer,
*  LED_Drive_segMBuffer, LED_Drive_segHBuffer.
*
* Parameters:
...
void LED_Drive_SetDisplayRAM(uint8 value, uint8 position) 
{
    ...
    if(position < LED_Drive_NUM_COM)
    {
        relPosition = position;
        #if(LED_Drive_SEG_DRIVE == LED_Drive_HIGH)
            LED_Drive_segLBuffer[relPosition] = value;
        #else
            LED_Drive_segLBuffer[relPosition] = ((uint8) ~value);
        #endif /* LED_Drive_SEG_DRIVE == LED_Drive_HIGH */
    }    
    else if (position < LED_Drive_TWO_NUM_COM)
    {
        #if (LED_Drive_NUM_SEG > 8u)
            relPosition = position - LED_Drive_NUM_COM;
            #if(LED_Drive_SEG_DRIVE == LED_Drive_HIGH)
                LED_Drive_segMBuffer[relPosition] = value;
            #else
                LED_Drive_segMBuffer[relPosition] = ((uint8) ~value);
            #endif /* LED_Drive_SEG_DRIVE == LED_Drive_HIGH */
        #endif /* (LED_Drive_NUM_SEG > 8u) */
    }
    else
    {
        #if (LED_Drive_NUM_SEG > 16u)
            relPosition = position - LED_Drive_TWO_NUM_COM;
            if(relPosition < LED_Drive_NUM_COM)
            {
                #if(LED_Drive_SEG_DRIVE == LED_Drive_HIGH)
                    LED_Drive_segHBuffer[relPosition] = value;
                #else
                    LED_Drive_segMBuffer[relPosition] = ((uint8) ~value);
                #endif /* LED_Drive_SEG_DRIVE == LED_Drive_HIGH */
            }
...
void LED_Drive_SetRC(uint8 row, uint8 column) 
{
    ...
    if(column < LED_Drive_NUM_COM)
    {
        if(row < 8u)
        {
            rowSeg = row;
            #if(LED_Drive_SEG_DRIVE == LED_Drive_HIGH)
                LED_Drive_segLBuffer[column] |= ((uint8)((uint8)0x01u << rowSeg));
            #else
                LED_Drive_segLBuffer[column] &= ((uint8)~((uint8)((uint8)0x01u << rowSeg)));
            #endif /* LED_Drive_SEG_DRIVE == LED_Drive_HIGH */
        }
        else if (row < 16u)
        {
                ...
                rowSeg = row - 8u;
                #if(LED_Drive_SEG_DRIVE == LED_Drive_HIGH)
                    LED_Drive_segMBuffer[column] |= ((uint8)((uint8)0x01u << rowSeg));
                #else
                    LED_Drive_segMBuffer[column] &= ((uint8)~((uint8)((uint8)0x01u << rowSeg)));
                #endif /* LED_Drive_SEG_DRIVE == LED_Drive_HIGH */
            #endif /* (LED_Drive_NUM_SEG > 8u) */
        }
        else
        {
                ...
                rowSeg = row - 16u;
                #if(LED_Drive_SEG_DRIVE == LED_Drive_HIGH)
                    LED_Drive_segHBuffer[column] |= ((uint8)((uint8)0x01u << rowSeg));
                #else
                    LED_Drive_segHBuffer[column] &= ((uint8)~((uint8)((uint8)0x01u << rowSeg)));
                #endif /* LED_Drive_SEG_DRIVE == LED_Drive_HIGH */
            #endif /* (LED_Drive_NUM_SEG > 16u) */
        }
    }
...
/*******************************************************************************
* Function Name: LED_Drive_ClearRC
********************************************************************************
*
* Summary:
...
void LED_Drive_ClearRC(uint8 row, uint8 column) 
{
    ...
    if(column < LED_Drive_NUM_COM)
    {
        if(row < 8u)
        {
            ...
            #if(LED_Drive_SEG_DRIVE == LED_Drive_HIGH)
                LED_Drive_segLBuffer[column] &= ((uint8)(~(uint8)rowSelect));
            #else
                LED_Drive_segLBuffer[column] |= rowSelect;
            #endif /* LED_Drive_SEG_DRIVE == LED_Drive_HIGH */
        }    
        else if (row < 16u)
        {
                ...
                rowSelect = ((uint8)((uint8)0x01u << rowSeg));
                #if(LED_Drive_SEG_DRIVE == LED_Drive_HIGH)
                    LED_Drive_segMBuffer[column] &= ((uint8)(~(uint8)rowSelect));
                #else
                    LED_Drive_segMBuffer[column] |= rowSelect;
                #endif /* LED_Drive_SEG_DRIVE == LED_Drive_HIGH */
            #endif /* (LED_Drive_NUM_SEG > 8u) */
        }
        else
        {
                ...
                rowSelect = ((uint8)((uint8)0x01u << rowSeg));
                #if(LED_Drive_SEG_DRIVE == LED_Drive_HIGH)
                    LED_Drive_segHBuffer[column] &= ((uint8)(~(uint8)rowSelect));
                #else
                    LED_Drive_segHBuffer[column] |= rowSelect;
                #endif /* LED_Drive_SEG_DRIVE == LED_Drive_HIGH */
            #endif /* (LED_Drive_NUM_SEG > 16u) */
        }
    }
...
/*******************************************************************************
* Function Name: LED_Drive_ToggleRC
********************************************************************************
*
* Summary:
...
uint8 LED_Drive_GetRC(uint8 row, uint8 column) 
{
    ...
    if(column < LED_Drive_NUM_COM)
    {
        ...
        {
            rowSeg = row;
            rowSegSelect = (uint8)((uint8)LED_Drive_segLBuffer[column] >> rowSeg);
        }    
        else if (row < 16u)
        {
            #if (LED_Drive_NUM_SEG > 8u) 
                rowSeg = row - 8u;
                rowSegSelect = (uint8)((uint8)LED_Drive_segMBuffer[column] >> rowSeg);
            #endif /* (LED_Drive_NUM_SEG > 8u) */
        }
        else
        {
            #if (LED_Drive_NUM_SEG > 16u)
                rowSeg = row - 16u;
                rowSegSelect = (uint8)((uint8)LED_Drive_segHBuffer[column] >> rowSeg);
            #endif /* (LED_Drive_NUM_SEG > 16u) */
        }
...
/*******************************************************************************
* Function Name: LED_Drive_ClearDisplay
********************************************************************************
*
* Summary:
...
{  
    ...
    {
        relPosition = position;
        LED_Drive_segLBuffer[relPosition] = LED_Drive_CLEAR;
    }    
    else if (position < LED_Drive_TWO_NUM_COM)
    {
        #if (LED_Drive_NUM_SEG > 8u)
            relPosition = position - LED_Drive_NUM_COM;
            LED_Drive_segMBuffer[relPosition] = LED_Drive_CLEAR;
        #endif /* (LED_Drive_NUM_SEG > 8u) */
    }
    else
    {
            ...
            if(relPosition < LED_Drive_NUM_COM)
            {
                LED_Drive_segHBuffer[relPosition] = LED_Drive_CLEAR;
            }
        #endif /* (LED_Drive_NUM_SEG > 16u) */
    }
}
...
/*******************************************************************************
* Function Name: LED_Drive_ClearDisplayAll
********************************************************************************
*
* Summary:
...
void LED_Drive_ClearDisplayAll(void) 
{
    ...
    for(position = 0u; position < LED_Drive_NUM_COM; position++)
    {
        LED_Drive_segLBuffer[position] = LED_Drive_CLEAR; 
        
        #if (LED_Drive_NUM_SEG > 8u) 
            LED_Drive_segMBuffer[position] = LED_Drive_CLEAR;
        #endif /* (LED_Drive_NUM_SEG > 8u) */
        
        #if (LED_Drive_NUM_SEG > 16u)
            LED_Drive_segHBuffer[position] = LED_Drive_CLEAR;
        #endif /* (LED_Drive_NUM_SEG > 16u) */
    }
}
...
/*******************************************************************************
* Function Name: LED_Drive_Write7SegNumberDec
********************************************************************************
*
* Summary:
...

{
    ...
    if(position < LED_Drive_NUM_COM)
    {
        choiceReg = LED_Drive_SEG_REG_LOW;
        relPosition = position;
    }
    else if (position < LED_Drive_TWO_NUM_COM)
    {
        choiceReg = LED_Drive_SEG_REG_MID;
        relPosition = position - LED_Drive_NUM_COM;
    }
    else
    {
        choiceReg = LED_Drive_SEG_REG_HIGH;
        relPosition = position - LED_Drive_TWO_NUM_COM;
    }
    
    /* Routine to extract the sign and to convert to unsigned */
    if(number >= LED_Drive_ZERO_NEG)
    {
        numberVal = (uint32)number;
        numberSign = LED_Drive_UNSIGNED;
    }
    else
    {
        numberVal = (uint32)(-number);
        numberSign = LED_Drive_SIGNED;
    }
    ...
    for(i = digits; i > 0u; i--)
    {
        ...
        bufArray[i-1u] = LED_Drive_EncodeNumber7Seg(selectVal);
        numberVal = numberVal / 10u;
    }
    
    /* Clean up the array and place sign if needed */
    if(alignment == LED_Drive_ZERO_PAD)
    {
        if(numberSign == LED_Drive_SIGNED)
        {
            /* Place sign on the MSB */
            bufArray[0u] = LED_Drive_EncodeChar7Seg('-');
        }
    }
    else
    {
        for(i = 0u; i < digits; i++)
        {
            if (bufArray[i] != LED_Drive_ZERO_7CHAR)
            {
                if(numberSign == LED_Drive_SIGNED)
                {
                    if(i == 0u)
                    {
                        /* Place sign on the MSB */
                        bufArray[i] = LED_Drive_EncodeChar7Seg('-');
                    }
                    else
                    {
                        /* Place sign before MSB */
                        bufArray[i-1u] = LED_Drive_EncodeChar7Seg('-');
                    }
                }
    ...
    if(alignment == LED_Drive_LEFT_ALIGN)
    {
        for(i = 0u; i < digits; i++)
        {
            if(bufArray[i] != LED_Drive_CLEAR)
            {
                if(relPosition < LED_Drive_NUM_COM)
                {
                    ...
                    switch(choiceReg)
                    {
                        case LED_Drive_SEG_REG_LOW :
                            LED_Drive_segLBuffer[relPosition] = bufArray[i];
                            break;
                        case LED_Drive_SEG_REG_MID :
                            #if (LED_Drive_NUM_SEG > 8u)
                                LED_Drive_segMBuffer[relPosition] = bufArray[i];
                            #endif /* (LED_Drive_NUM_SEG > 8u) */
                            break;
                        case LED_Drive_SEG_REG_HIGH :
                            #if (LED_Drive_NUM_SEG > 16u)
                                LED_Drive_segHBuffer[relPosition] = bufArray[i];
                            #endif /* (LED_Drive_NUM_SEG > 16u) */
                            break;
                        default :
                            LED_Drive_segLBuffer[relPosition] = bufArray[i];
    ...
    else
    {
        ...
        for(i = digits; i > 0u; i--)
        {
            if(((i + relPosition) - 1u) < LED_Drive_NUM_COM)
            {
                /* Decide which row to use */
                switch(choiceReg)
                {
                    case LED_Drive_SEG_REG_LOW :
                        LED_Drive_segLBuffer[(i + relPosition) - 1u] = bufArray[i-1u];
                        break;
                    case LED_Drive_SEG_REG_MID :
                        #if (LED_Drive_NUM_SEG > 8u)
                            LED_Drive_segMBuffer[(i + relPosition) - 1u] = bufArray[i-1u];
                        #endif /* (LED_Drive_NUM_SEG > 8u) */
                        break;
                    case LED_Drive_SEG_REG_HIGH :
                        #if (LED_Drive_NUM_SEG > 16u)
                            LED_Drive_segHBuffer[(i + relPosition) - 1u] = bufArray[i-1u];
                        #endif /* (LED_Drive_NUM_SEG > 16u) */
                        break;
                    default :
                        LED_Drive_segLBuffer[(i + relPosition) - 1u] = bufArray[i-1u];
...
/*******************************************************************************
* Function Name: LED_Drive_Write7SegNumberHex
********************************************************************************
*
* Summary:
...

{
    ...
    for(i = digits; i > 0u; i--)
    {
        selectVal = (uint8)((uint8)numberVal & LED_Drive_NIBBLE_MASK);
        bufArray[i-1u] = LED_Drive_EncodeNumber7Seg(selectVal);
        numberVal = (uint32)((uint32)numberVal >> LED_Drive_SHIFT_NIBBLE);
    }
...
/*******************************************************************************
* Function Name: LED_Drive_WriteString7Seg
********************************************************************************
*
* Summary:
...

{
    ...
    if(position < LED_Drive_NUM_COM)
    {
        ...
        for(relPosition = position; relPosition < LED_Drive_NUM_COM; relPosition++)
        {
            if(relPosition < sizeLimit)
            {
                LED_Drive_segLBuffer[relPosition] = LED_Drive_EncodeChar7Seg(character[i]);
                i++;
            }
        }
    }    
    ...
    {
        #if (LED_Drive_NUM_SEG > 8u) 
            sizeLimit = ((uint8)(strlen(character)) + position) - LED_Drive_NUM_COM;
            for(relPosition = position - LED_Drive_NUM_COM;
                relPosition < LED_Drive_NUM_COM; 
                relPosition++)
            {
                if(relPosition < sizeLimit)
                {
                    LED_Drive_segMBuffer[relPosition] = LED_Drive_EncodeChar7Seg(character[i]);
                    i++;
                }
            }
        #endif /* (LED_Drive_NUM_SEG > 8u) */
    ...
    {
        #if (LED_Drive_NUM_SEG > 16u)
            sizeLimit = ((uint8)(strlen(character)) + position) - LED_Drive_TWO_NUM_COM;
            for(relPosition = position - LED_Drive_TWO_NUM_COM;
                relPosition < LED_Drive_NUM_COM; 
                relPosition++)
            {
                if(relPosition < sizeLimit)
                {
                    LED_Drive_segHBuffer[relPosition] = LED_Drive_EncodeChar7Seg(character[i]);
                    i++;
                }    
            }
        #endif /* (LED_Drive_NUM_SEG > 16u) */
...
/*******************************************************************************
* Function Name: LED_Drive_PutChar7Seg
********************************************************************************
*
* Summary:
...

{
    ...
    {
        relPosition = position;
        LED_Drive_segLBuffer[relPosition] = LED_Drive_EncodeChar7Seg(character);
    }    
    else if (position < LED_Drive_TWO_NUM_COM)
    {
        #if (LED_Drive_NUM_SEG > 8u) 
            relPosition = position - LED_Drive_NUM_COM;
            LED_Drive_segMBuffer[relPosition] = LED_Drive_EncodeChar7Seg(character);
        #endif /* (LED_Drive_NUM_SEG > 8u) */
    }
    else
    {
            ...
            if(relPosition < LED_Drive_NUM_COM)
            {
                LED_Drive_segHBuffer[relPosition] = LED_Drive_EncodeChar7Seg(character);
            }
        #endif /* (LED_Drive_NUM_SEG > 16u) */
    }
}
...
/*******************************************************************************
* Function Name: LED_Drive_Write7SegDigitDec
********************************************************************************
*
* Summary:
...

{
    ...
    if (digit < 10u)
    {
        ...
        {
            relPosition = position;
            LED_Drive_segLBuffer[relPosition] = LED_Drive_EncodeNumber7Seg(digit);
        }    
        else if (position < LED_Drive_TWO_NUM_COM)
        {
            #if (LED_Drive_NUM_SEG > 8u)
                relPosition = position - LED_Drive_NUM_COM;
                LED_Drive_segMBuffer[relPosition] = LED_Drive_EncodeNumber7Seg(digit);
            #endif /* (LED_Drive_NUM_SEG > 8u) */
        } 
        else
        {
            #if (LED_Drive_NUM_SEG > 16u)
                relPosition = position - LED_Drive_TWO_NUM_COM;
                if(relPosition < LED_Drive_TWO_NUM_COM)
                {
                    LED_Drive_segHBuffer[relPosition] = LED_Drive_EncodeNumber7Seg(digit);
                }
            #endif /* (LED_Drive_NUM_SEG > 16u) */
        }
    }
...
/*******************************************************************************
* Function Name: LED_Drive_Write7SegDigitHex
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: LED_Drive_Write14SegNumberDec
********************************************************************************
*
* Summary:
...

{
    ...
    {
        selectVal = (uint8)(numberVal % 10u);
        bufArray[i-1u] = LED_Drive_EncodeNumber14Seg(selectVal);
        numberVal = numberVal / 10u;
    }
    ...
    if(alignment == LED_Drive_ZERO_PAD)
    {
        ...
        {
            /* Place sign on the MSB */
            bufArray[0u] = LED_Drive_EncodeChar14Seg('-');
        }
    }
    else
    {
        for(i = 0u; i < digits; i++)
        {
            if (bufArray[i] != LED_Drive_ZERO_14CHAR)
            {
                if(numberSign == LED_Drive_SIGNED)
                {
                    if(i == 0u)
                    {
                        /* Place sign on the MSB */
                        bufArray[i] = LED_Drive_EncodeChar14Seg('-');
                    }
                    else
                    {
                        /* Place sign before MSB */
                        bufArray[i-1u] = LED_Drive_EncodeChar14Seg('-');
                    }
                }
                /* Exit when encountering non-zero number */
                break;
    ...
    if(alignment == LED_Drive_LEFT_ALIGN)
    {
        for(i = 0u; i < digits; i++)
        {
            if(bufArray[i] != LED_Drive_CLEAR_16)
            {
                if(relPosition < LED_Drive_NUM_COM)
                {
                    /* Only show displays that are not empty */
                    LED_Drive_segLBuffer[relPosition] = LO8(bufArray[i]);
                #if (LED_Drive_NUM_SEG > 8u)
                    LED_Drive_segMBuffer[relPosition] = HI8(bufArray[i]);
                #endif /* (LED_Drive_NUM_SEG > 8u) */
                }
                relPosition++;
            }
    ...
    else
    {
        for(i = digits; i > 0u; i--)
        {
            if(((i + relPosition) - 1u) < LED_Drive_NUM_COM)
            {
                LED_Drive_segLBuffer[(i + relPosition) - 1u] = LO8(bufArray[i-1u]);
            #if (LED_Drive_NUM_SEG > 8u)
                LED_Drive_segMBuffer[(i + relPosition) - 1u] = HI8(bufArray[i-1u]);
            #endif /* (LED_Drive_NUM_SEG > 8u) */
            }
        }  
    }
...
/*******************************************************************************
* Function Name: LED_Drive_Write14SegNumberHex
********************************************************************************
*
* Summary:
*  Displays a hexadecimal number up to 8 characters long, starting at 'position'
...
/*******************************************************************************
* Function Name: LED_Drive_WriteString14Seg
********************************************************************************
*
* Summary:
...

{
    ...
    for(currPosition = position; currPosition < LED_Drive_NUM_COM; currPosition++)
    {
        if(currPosition < sizeLimit)
        {
            charDisplay = LED_Drive_EncodeChar14Seg(character[i]);
            LED_Drive_segLBuffer[currPosition] = LO8(charDisplay);
            #if (LED_Drive_NUM_SEG > 8u)
            LED_Drive_segMBuffer[currPosition] = HI8(charDisplay);
            #endif /* (LED_Drive_NUM_SEG > 8u) */
            i++;
        }
    }
...
/*******************************************************************************
* Function Name: LED_Drive_PutChar14Seg
********************************************************************************
*
* Summary:
*  Displays an ascii encoded character at 'position'. This function can display
...

{
    ...
    if(position < LED_Drive_NUM_COM)
    {
        charDisplay = LED_Drive_EncodeChar14Seg(character);
        LED_Drive_segLBuffer[position] = LO8(charDisplay);
    #if (LED_Drive_NUM_SEG > 8u)
        LED_Drive_segMBuffer[position] = HI8(charDisplay);
    #endif /* (LED_Drive_NUM_SEG > 8u) */
    }
}
...
/*******************************************************************************
* Function Name: LED_Drive_Write14SegDigitDec
********************************************************************************
*
* Summary:
*  Displays a single digit on the specified display. The number in 'digit' 
...

{
    ...
    if((digit < 10u) && (position < LED_Drive_NUM_COM))
    {
        charDisplay = LED_Drive_EncodeNumber14Seg(digit);
        LED_Drive_segLBuffer[position] = LO8(charDisplay);
    #if (LED_Drive_NUM_SEG > 8u)
        LED_Drive_segMBuffer[position] = HI8(charDisplay);
    #endif /* (LED_Drive_NUM_SEG > 8u) */
...
/*******************************************************************************
* Function Name: LED_Drive_Write14SegDigitHex
********************************************************************************
*
* Summary:
*  Displays a single digit at the specified position. The number in 'digit' 
...
/*******************************************************************************
* Function Name: LED_Drive_Write16SegNumberDec
********************************************************************************
*
* Summary:
*  Displays a signed integer up to 8 characters long, starting at 'position' 
...

{
    ...
    {
        selectVal = (uint8)(numberVal % 10u);
        bufArray[i-1u] = LED_Drive_EncodeNumber16Seg(selectVal);
        numberVal = numberVal / 10u;
    }
    ...
    if(alignment == LED_Drive_ZERO_PAD)
    {
        ...
        {
            /* Place sign on the MSB */
            bufArray[0u] = LED_Drive_EncodeChar16Seg('-');
        }
    }
    else
    {
        for(i = 0u; i < digits; i++)
        {
            if (bufArray[i] != LED_Drive_ZERO_16CHAR)
            {
                if(numberSign == LED_Drive_SIGNED)
                {
                    if(i == 0u)
                    {
                        /* Place sign on the MSB */
                        bufArray[i] = LED_Drive_EncodeChar16Seg('-');
                    }
                    else
                    {
                        /* Place sign before MSB */
                        bufArray[i-1u] = LED_Drive_EncodeChar16Seg('-');
                    }
                }
                /* Exit when encountering non-zero number */
                break;
...
/*******************************************************************************
* Function Name: LED_Drive_Write16SegNumberHex
********************************************************************************
*
* Summary:
*  Displays a hexadecimal number up to 8 characters long, starting at 'position'
...
/*******************************************************************************
* Function Name: LED_Drive_WriteString16Seg
********************************************************************************
*
* Summary:
*  Displays a null terminated string starting at 'position' and ending at either
...

{
    ...
    for(currPosition = position; currPosition < LED_Drive_NUM_COM; currPosition++)
    {
        if(currPosition < sizeLimit)
        {
            charDisplay = LED_Drive_EncodeChar16Seg(character[i]);
            LED_Drive_segLBuffer[currPosition] = LO8(charDisplay);
            #if (LED_Drive_NUM_SEG > 8u)
                LED_Drive_segMBuffer[currPosition] = HI8(charDisplay);
            #endif /* (LED_Drive_NUM_SEG > 8u) */
...
/*******************************************************************************
* Function Name: LED_Drive_PutChar16Seg
********************************************************************************
*
* Summary:
*  Displays an ascii encoded character at 'position'. This function can display 
...

{
    ...
    if(position < LED_Drive_NUM_COM)
    {
        charDisplay = LED_Drive_EncodeChar16Seg(character);
        LED_Drive_segLBuffer[position] = LO8(charDisplay);
        #if (LED_Drive_NUM_SEG > 8u)
            LED_Drive_segMBuffer[position] = HI8(charDisplay);
        #endif /* (LED_Drive_NUM_SEG > 8u) */
...
/*******************************************************************************
* Function Name: LED_Drive_Write16SegDigitDec
********************************************************************************
*
* Summary:
*  Displays a single digit on the specified display. The number in 'digit' 
...

{
    ...
    if((digit < 10u) && (position < LED_Drive_NUM_COM))
    {
        charDisplay = LED_Drive_EncodeNumber16Seg(digit);
        LED_Drive_segLBuffer[position] = LO8(charDisplay);
        #if (LED_Drive_NUM_SEG > 8u)
            LED_Drive_segMBuffer[position] = HI8(charDisplay);
        #endif /* (LED_Drive_NUM_SEG > 8u) */
...
/*******************************************************************************
* Function Name: LED_Drive_Write16SegDigitHex
********************************************************************************
*
* Summary:
*  Displays a single digit at the specified position. The number in 'digit' 
...
/*******************************************************************************
* Function Name: LED_Drive_PutDecimalPoint
********************************************************************************
*
* Summary:
...

{
    ...
    value = LED_Drive_DEC_POINT_VALUE;
    ...
    if(position < LED_Drive_NUM_COM)
    {
        ...
        {
            #if(LED_Drive_SEG_DRIVE == LED_Drive_HIGH)
                LED_Drive_segLBuffer[relPosition] &= ((uint8)(~(uint8)value));
            #else
                LED_Drive_segLBuffer[relPosition] |= ((uint8)(~(uint8)value));
            #endif /* (LED_Drive_SEG_DRIVE == LED_Drive_HIGH) */
        }
    ...
    else if (position < LED_Drive_TWO_NUM_COM)
    {
            ...
            {
                #if(LED_Drive_SEG_DRIVE == LED_Drive_HIGH)
                    LED_Drive_segMBuffer[relPosition] &= ((uint8)(~(uint8)value));
                #else
                    LED_Drive_segMBuffer[relPosition] |= ((uint8)(~(uint8)value));
                #endif /* (LED_Drive_SEG_DRIVE == LED_Drive_HIGH) */
            }
    ...
    else
    {
            ...
            if(relPosition < LED_Drive_NUM_COM)
            {
                ...
                {
                    #if(LED_Drive_SEG_DRIVE == LED_Drive_HIGH)
                        LED_Drive_segHBuffer[relPosition] &= ((uint8)(~(uint8)value));
                    #else
                        LED_Drive_segHBuffer[relPosition] |= ((uint8)(~(uint8)value));
                    #endif /* (LED_Drive_SEG_DRIVE == LED_Drive_HIGH) */
                }
...
/*******************************************************************************
* Function Name: LED_Drive_GetDecimalPoint
********************************************************************************
*
* Summary:
...
uint8 LED_Drive_GetDecimalPoint(uint8 position) 
{
    ...
    {
        relPosition = position;
        decPointVal = (uint8)((uint8)(LED_Drive_segLBuffer[relPosition]) >> LED_Drive_DEC_POINT);
    } 
    else if (position < LED_Drive_TWO_NUM_COM)
    {
        #if (LED_Drive_NUM_SEG > 8u) 
            relPosition = position - LED_Drive_NUM_COM;
            decPointVal = (uint8)((uint8)(LED_Drive_segMBuffer[relPosition]) >> LED_Drive_DEC_POINT);
        #endif /* (LED_Drive_NUM_SEG > 8u) */
    }
    else
    {
            ...
            if(relPosition < LED_Drive_NUM_COM)
            {
                decPointVal = (uint8)((uint8)(LED_Drive_segHBuffer[relPosition]) >> LED_Drive_DEC_POINT);
            }
        #endif /* (LED_Drive_NUM_SEG > 16u) */
    }
    
    #if(LED_Drive_SEG_DRIVE == LED_Drive_HIGH)
        value = (uint8)(decPointVal & LED_Drive_DEC_POINT_MASK);
    #else
        value = (uint8)((uint8)((uint8)~decPointVal) & LED_Drive_DEC_POINT_MASK);
    #endif /* (LED_Drive_SEG_DRIVE == LED_Drive_HIGH) */
...
/*******************************************************************************
* Function Name: LED_Drive_EncodeNumber7Seg
********************************************************************************
*
* Summary:
...
uint8 LED_Drive_EncodeNumber7Seg(uint8 number) 
{
    ...
    #if(LED_Drive_SEG_DRIVE == LED_Drive_HIGH)
        static const uint8 CYCODE LED_Drive_7SegDigits[16] =
              /*--------------------------------------------------------*/
              /*  '0'    '1'    '2'    '3'    '4'    '5'    '6'    '7'  */
              /*--------------------------------------------------------*/
    ...
    #endif /*  LED_Drive_SEG_DRIVE */

    uint8 valNum;
    uint8 valHex;
    
    valNum = (uint8)(number & LED_Drive_NIBBLE_MASK);
    
    valHex = LED_Drive_7SegDigits[valNum];
...
/*******************************************************************************
* Function Name: LED_Drive_EncodeChar7Seg
********************************************************************************
*
* Summary:
...
uint8 LED_Drive_EncodeChar7Seg(char8 input)  
{
    /* 7-segment ASCII LUT */
    #if(LED_Drive_SEG_DRIVE == LED_Drive_HIGH)
        static const uint8 CYCODE LED_Drive_7SegASCII[6][16] =
              {
              /*--------------------------------------------------------*/
              /* 'Space' '!'    '"'    '#'    '$'    '%'    '&'    '''  */
              /*--------------------------------------------------------*/
    ...
    valLow = (uint8)((uint8)input & LED_Drive_NIBBLE_MASK);
    
    /* valHigh used to access the row of LUT */
    valHigh = (uint8)((uint8)input >> LED_Drive_SHIFT_NIBBLE);
    
    /* Check if the high nibble is within valid ASCII range */
    if ((valHigh > LED_Drive_SEG_ASCII_LIM_LO) && (valHigh < LED_Drive_SEG_ASCII_LIM_HI))
    {
        valHigh = valHigh - LED_Drive_SEG_ASCII_OFFSET;
        valHex = LED_Drive_7SegASCII[valHigh][valLow];
    }
    else
    {
        valHex = LED_Drive_CLEAR;
    }
...
/*******************************************************************************
* Function Name: LED_Drive_EncodeNumber14Seg
********************************************************************************
*
* Summary:
...
uint16 LED_Drive_EncodeNumber14Seg(uint8 number) 
{
        ...
        static const uint8 CYCODE LED_Drive_14SegDigitsLow[16] =
              /*--------------------------------------------------------*/
              /*  '0'    '1'    '2'    '3'    '4'    '5'    '6'    '7'  */
              /*--------------------------------------------------------*/
        ...
        static const uint8 CYCODE LED_Drive_14SegDigitsHigh[16] =
              /*--------------------------------------------------------*/
              /*  '0'    '1'    '2'    '3'    '4'    '5'    '6'    '7'  */
              /*--------------------------------------------------------*/
    ...
    valHigh = (uint16)((uint16)LED_Drive_14SegDigitsHigh[valNum] << LED_Drive_SHIFT_BYTE);
    valLow = (uint16)LED_Drive_14SegDigitsLow[valNum];
...
/*******************************************************************************
* Function Name: LED_Drive_EncodeChar14Seg
********************************************************************************
*
* Summary:
...
uint16 LED_Drive_EncodeChar14Seg(char8 input) 
{
        ...
        static const uint8 CYCODE LED_Drive_14SegASCIILow[6][16] =
              {
              /*--------------------------------------------------------*/
              /* 'Space' '!'    '"'    '#'    '$'    '%'    '&'    '''  */
              /*--------------------------------------------------------*/
        ...
        static const uint8 CYCODE LED_Drive_14SegASCIIHigh[6][16] =
              {
              /*--------------------------------------------------------*/
              /* 'Space' '!'    '"'    '#'    '$'    '%'    '&'    '''  */
              /*--------------------------------------------------------*/
    ...
    if ((valHigh > LED_Drive_SEG_ASCII_LIM_LO) && (valHigh < LED_Drive_SEG_ASCII_LIM_HI))
    {
        ...
        /* Get the 14-seg hex value from high and low byte LUTs */
        valHexHigh = (uint16)((uint16)LED_Drive_14SegASCIIHigh[valHigh][valLow] << LED_Drive_SHIFT_BYTE);
        valHexLow = (uint16)LED_Drive_14SegASCIILow[valHigh][valLow];
        
        valHex = valHexHigh | valHexLow;
    }
    else
    {
        valHex = LED_Drive_CLEAR_16;
    }
...
/*******************************************************************************
* Function Name: LED_Drive_EncodeNumber16Seg
********************************************************************************
*
* Summary:
...
uint16 LED_Drive_EncodeNumber16Seg(uint8 number) 
{
        ...
        static const uint8 CYCODE LED_Drive_16SegDigitsLow[16] =
              /*--------------------------------------------------------*/
              /*  '0'    '1'    '2'    '3'    '4'    '5'    '6'    '7'  */
              /*--------------------------------------------------------*/
        ...
        static const uint8 CYCODE LED_Drive_16SegDigitsHigh[16] =
              /*--------------------------------------------------------*/
              /*  '0'    '1'    '2'    '3'    '4'    '5'    '6'    '7'  */
              /*--------------------------------------------------------*/
    ...
    valHigh = (uint16)((uint16)LED_Drive_16SegDigitsHigh[valNum] << LED_Drive_SHIFT_BYTE);
    valLow = (uint16)LED_Drive_16SegDigitsLow[valNum];
...
/*******************************************************************************
* Function Name: LED_Drive_EncodeChar16Seg
********************************************************************************
*
* Summary:
...
uint16 LED_Drive_EncodeChar16Seg(char8 input) 
{
        ...
        static const uint8 CYCODE LED_Drive_16SegASCIILow[6][16] =
              {
              /*--------------------------------------------------------*/
              /* 'Space' '!'    '"'    '#'    '$'    '%'    '&'    '''  */
              /*--------------------------------------------------------*/
        ...
        static const uint8 CYCODE LED_Drive_16SegASCIIHigh[6][16] =
              {
              /*--------------------------------------------------------*/
              /* 'Space' '!'    '"'    '#'    '$'    '%'    '&'    '''  */
              /*--------------------------------------------------------*/
    ...
    if ((valHigh > LED_Drive_SEG_ASCII_LIM_LO) && (valHigh < LED_Drive_SEG_ASCII_LIM_HI))
    {
        ...
        /* Get the 16-seg hex value from high and low byte LUTs */
        valHexHigh = (uint16)((uint16)LED_Drive_16SegASCIIHigh[valHigh][valLow] << LED_Drive_SHIFT_BYTE);
        valHexLow = (uint16)LED_Drive_16SegASCIILow[valHigh][valLow];
...
/*******************************************************************************
* Function Name: LED_Drive_SetBrightness
********************************************************************************
*
* Summary:
...
{   
    #if(LED_Drive_COM_DRIVE == LED_Drive_HIGH)
        static const uint8 CYCODE LED_Drive_tempCom[16] = {0x00u, 0x01u, 0x00u, 0x02u, 0x00u, 0x04u,
                                                                  0x00u, 0x08u, 0x00u, 0x10u, 0x00u, 0x20u,
                                                                  0x00u, 0x40u, 0x00u, 0x80u};
    #else
        static const uint8 CYCODE LED_Drive_tempCom[16] = {0xFFu, 0xFEu, 0xFFu, 0xFDu, 0xFFu, 0xFBu,
    ...
    if(bright == 0u)
    {
        ...
        LED_Drive_comBuffer[(uint8)((uint8)position << 1u) + 1u] = LED_Drive_CLEAR;
    }
    else
    {
        /* Restore the commons */
        LED_Drive_comBuffer[(uint8)((uint8)position << 1u) + 1u]
            = LED_Drive_tempCom[(uint8)((uint8)position << 1u) + 1u];
    }
    
    /* Update the brightness register */
    LED_Drive_brightnessReg[position] = bright;
}


/*******************************************************************************
* Function Name: LED_Drive_GetBrightness
********************************************************************************
*
* Summary:
...
{
    return(LED_Drive_brightnessReg[position]);
}
