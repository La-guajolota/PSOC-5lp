/*******************************************************************************
* File Name: LED_Drive_ClkInternal.c
* Version 2.20
*
*  Description:
...
#include "LED_Drive_ClkInternal.h"
...
/*******************************************************************************
* Function Name: LED_Drive_ClkInternal_Start
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LED_Drive_ClkInternal_Start(void) 
{
    /* Set the bit to enable the clock. */
    LED_Drive_ClkInternal_CLKEN |= LED_Drive_ClkInternal_CLKEN_MASK;
	LED_Drive_ClkInternal_CLKSTBY |= LED_Drive_ClkInternal_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: LED_Drive_ClkInternal_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LED_Drive_ClkInternal_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    LED_Drive_ClkInternal_CLKEN &= (uint8)(~LED_Drive_ClkInternal_CLKEN_MASK);
	LED_Drive_ClkInternal_CLKSTBY &= (uint8)(~LED_Drive_ClkInternal_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: LED_Drive_ClkInternal_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LED_Drive_ClkInternal_StopBlock(void) 
{
    if ((LED_Drive_ClkInternal_CLKEN & LED_Drive_ClkInternal_CLKEN_MASK) != 0u)
    {
...
#if defined(LED_Drive_ClkInternal__CFG3)
        CLK_DIST_AMASK = LED_Drive_ClkInternal_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = LED_Drive_ClkInternal_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* LED_Drive_ClkInternal__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(LED_Drive_ClkInternal_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(LED_Drive_ClkInternal_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: LED_Drive_ClkInternal_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LED_Drive_ClkInternal_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        LED_Drive_ClkInternal_CLKSTBY &= (uint8)(~LED_Drive_ClkInternal_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: LED_Drive_ClkInternal_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LED_Drive_ClkInternal_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = LED_Drive_ClkInternal_GetSourceRegister();
    uint16 oldDivider = LED_Drive_ClkInternal_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = LED_Drive_ClkInternal_CLKEN & LED_Drive_ClkInternal_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(LED_Drive_ClkInternal_DIV_PTR, clkDivider);
                LED_Drive_ClkInternal_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                LED_Drive_ClkInternal_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(LED_Drive_ClkInternal_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((LED_Drive_ClkInternal_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(LED_Drive_ClkInternal_DIV_PTR, clkDivider);
				LED_Drive_ClkInternal_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: LED_Drive_ClkInternal_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 LED_Drive_ClkInternal_GetDividerRegister(void) 
{
    return CY_GET_REG16(LED_Drive_ClkInternal_DIV_PTR);
}


/*******************************************************************************
* Function Name: LED_Drive_ClkInternal_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LED_Drive_ClkInternal_SetModeRegister(uint8 modeBitMask) 
{
    LED_Drive_ClkInternal_MOD_SRC |= modeBitMask & (uint8)LED_Drive_ClkInternal_MODE_MASK;
}


/*******************************************************************************
* Function Name: LED_Drive_ClkInternal_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LED_Drive_ClkInternal_ClearModeRegister(uint8 modeBitMask) 
{
    LED_Drive_ClkInternal_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(LED_Drive_ClkInternal_MODE_MASK));
}


/*******************************************************************************
* Function Name: LED_Drive_ClkInternal_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 LED_Drive_ClkInternal_GetModeRegister(void) 
{
    return LED_Drive_ClkInternal_MOD_SRC & (uint8)(LED_Drive_ClkInternal_MODE_MASK);
}


/*******************************************************************************
* Function Name: LED_Drive_ClkInternal_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LED_Drive_ClkInternal_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = LED_Drive_ClkInternal_GetDividerRegister();
    uint8 oldSrc = LED_Drive_ClkInternal_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        LED_Drive_ClkInternal_MOD_SRC |= CYCLK_SSS;
        LED_Drive_ClkInternal_MOD_SRC =
            (LED_Drive_ClkInternal_MOD_SRC & (uint8)(~LED_Drive_ClkInternal_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: LED_Drive_ClkInternal_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 LED_Drive_ClkInternal_GetSourceRegister(void) 
{
    return LED_Drive_ClkInternal_MOD_SRC & LED_Drive_ClkInternal_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: LED_Drive_ClkInternal_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LED_Drive_ClkInternal_SetPhaseRegister(uint8 clkPhase) 
{
    LED_Drive_ClkInternal_PHASE = clkPhase & LED_Drive_ClkInternal_PHASE_MASK;
}


/*******************************************************************************
* Function Name: LED_Drive_ClkInternal_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 LED_Drive_ClkInternal_GetPhaseRegister(void) 
{
    return LED_Drive_ClkInternal_PHASE & LED_Drive_ClkInternal_PHASE_MASK;
}
